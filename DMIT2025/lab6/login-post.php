<?php  
	if (isset($_POST['login-btn'])) {
		...
		if ($email && $password) {
			$login_sql = "SELECT u_id, first_name, password FROM a1_users WHERE user_name = '$email' OR email = '$email'";
			...
			if ($login_result->num_rows > 0) {
				...
				if (password_verify($password, $row['password'])) {
					...
					$update_sql = "UPDATE a1_users SET date_last_login = NOW() WHERE u_id = " . $_SESSION['user_id'];
