<?php  
	...
	if (isset($_POST['change-btn'])) {
		...
		if ($email && $current && $newpass) {
			...
			{
				$check_sql = "SELECT * FROM a1_users WHERE email = '$email' OR user_name = '$email'";
				...
				if ($check_details->num_rows > 0){
					...
					if(password_verify($current, $row['password']) && $_POST["newpass"] == $_POST["confirm"] ) {
						...
						$update_pass_sql = "UPDATE a1_users SET password = '$hash' WHERE email = '$email' OR user_name = '$email'";
